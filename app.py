...
def main():
    ...
    # side-bar
    with st.sidebar:
        st.sidebar.title(':blue[QUICK INTRODUCTION]')
        st.markdown('**PCB fabrication process** often suffers from defects, '
                        'such as \n - missing holes ')
        ...
        st.markdown('\n :red[These defects, if not detected before electronic components are fabricated onto the board, '
                    'poses serious dangers to the components and its use]. \n :green[Hence the need for this software]')
    
    # Main page
    st.title(':green[PCB Defects Detection Software]')
    # dept_img=['./fut.jpeg', './mce.jpeg']
    # st.image(dept_img, use_column_width= False,  caption=['Proudly developed by the department of Mechatronics Engineering FUTMINNA']* len(dept_img))
    ...
    test_options = ["./01.JPG", "./01_missing_hole.jpg", "./01_mouse_bite.jpg", "./04_spurious_copper.jpg",
